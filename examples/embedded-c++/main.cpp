//
// Created by Azim Afroozeh on 2/28/21.
//
#include "duckdb.hpp"
...
int main(int argc, char **argv) {
    ...
    //#ifdef test
    //    con.Query("CREATE TABLE test(i TINYINT NOT NULL); INSERT INTO test SELECT (i) AS i FROM range(0, 100)
    //    tbl(i);");
    //#else
    //    con.Query("CREATE TABLE test(i TINYINT NOT NULL); INSERT INTO test SELECT (i / 300000000) AS i FROM range(0,
    //    1024) tbl(i);");
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if ((arg == "-h") || (arg == "--help")) {
            ...
            if (arg == "3") {
                ...
                // Load data
                auto result =
                    con.Query("explain CREATE TABLE tbl AS SELECT (i % 2)::TINYINT i FROM range(4096) tbl(i);");
