...
static void printf_function(DataChunk &args, ExpressionState &state, Vector &result) {
	...
	for (idx_t i = 0; i < args.ColumnCount(); i++) {
		switch (args.data[i].buffer->vector_type) {
	...
	idx_t count = result.buffer->vector_type == VectorType::CONSTANT_VECTOR ? 1 : args.size();

	auto format_data = FlatVector::GetData<string_t>(format_string);
	auto result_data = FlatVector::GetData<string_t>(result);
	for (idx_t idx = 0; idx < count; idx++) {
		if (result.buffer->vector_type == VectorType::FLAT_VECTOR && FlatVector::IsNull(result, idx)) {
			// this entry is NULL: skip it
			continue;
		}

		// first fetch the format string
		auto fmt_idx = format_string.buffer->vector_type == VectorType::CONSTANT_VECTOR ? 0 : idx;
		...
		for (idx_t col_idx = 1; col_idx < args.ColumnCount(); col_idx++) {
			...
			idx_t arg_idx = col.buffer->vector_type == VectorType::CONSTANT_VECTOR ? 0 : idx;
			switch (col.buffer->type.id()) {
			case LogicalTypeId::BOOLEAN: {
			...
			default:
				throw InvalidInputException("Unsupported type for format: \"%s\"!", col.buffer->type.ToString());
			}
		}
