...
static unique_ptr<FunctionData> bind(ClientContext &context, vector<Value> &inputs,
                                     unordered_map<string, Value> &named_parameters, vector<LogicalType> &return_types,
                                     vector<string> &names) {
...
static void ExtractExpressions(ExpressionInformation &info, DataChunk &output, int *index, int op_id, int *exp_id,
	...
	if (info.hasfunction) {
		SetValue(output, *(index), op_id, *exp_id, ", Function: " + info.function_name,
...
static void function1(ClientContext &context, const FunctionData *bind_data_p, FunctionOperatorData *operator_state,
                      DataChunk &output) {
...
void PragmaLastProfilingOutput::RegisterFunction(BuiltinFunctions &set) {
	set.AddFunction(TableFunction("pragma_last_profiling_output", {}, function1, bind, init));
}
