...
static void ExtractExpressions(ExpressionInformation &info, DataChunk &output, int* index, int op_id, int* exp_id, int sample_tuples_count) {
	if(info.hasfunction){
		SetValue(output, *(index), op_id, *exp_id, ", Function: " + info.function_name, double(info.time) / sample_tuples_count);
		(*index) = *index + 1;
        (*exp_id) = *exp_id + 1;
	}
    ...
    for (auto &child : info.children) {
		ExtractExpressions(*child, output, index, op_id, exp_id, sample_tuples_count);
    }
    return;
}
		...
		for(auto op: context.prev_profiler.tree_map){
			...
			if(op.second->info.has_executor) {
				for (auto &info : op.second->info.executors_info->roots) {
					ExtractExpressions(*info, output, &(total_counter), operator_counter, &(expression_counter), op.second->info.executors_info->sample_tuples_count);
				}
			}
			operator_counter++;
		}
