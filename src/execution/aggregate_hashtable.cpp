...
void GroupedAggregateHashTable::FetchAggregates(DataChunk &groups, DataChunk &result) {
	...
	for (idx_t i = 0; i < result.ColumnCount(); i++) {
		D_ASSERT(result.data[i].buffer->type == payload_types[i]);
	}
...
void GroupedAggregateHashTable::ScatterGroups(DataChunk &groups, unique_ptr<VectorData[]> &group_data,
	...
	for (idx_t grp_idx = 0; grp_idx < groups.ColumnCount(); grp_idx++) {
		...
		auto type_size = GetTypeIdSize(data.buffer->type.InternalType());
...
idx_t GroupedAggregateHashTable::FindOrCreateGroupsInternal(DataChunk &groups, Vector &group_hashes, Vector &addresses,
	...
	D_ASSERT(capacity - entries >= groups.size());
	D_ASSERT(group_hashes.buffer->type == LogicalType::HASH);

	group_hashes.Normalify(groups.size());
	const auto group_hashes_ptr = FlatVector::GetData<hash_t>(group_hashes);

	D_ASSERT(ht_offsets.buffer->vector_type == VectorType::FLAT_VECTOR);
	D_ASSERT(ht_offsets.buffer->type == LogicalType::BIGINT);

	D_ASSERT(addresses.buffer->type == LogicalType::POINTER);
	...
	D_ASSERT(hash_salts.buffer->type == LogicalType::SMALLINT);
...
void GroupedAggregateHashTable::FlushMove(Vector &source_addresses, Vector &source_hashes, idx_t count) {
	D_ASSERT(source_addresses.buffer->type == LogicalType::POINTER);
	D_ASSERT(source_hashes.buffer->type == LogicalType::HASH);
