...
public:
	PhysicalCrossProductOperatorState(ExecutionContext &execution_context, PhysicalOperator &op, PhysicalOperator *left, PhysicalOperator *right)
...
unique_ptr<PhysicalOperatorState> PhysicalCrossProduct::GetOperatorState(ExecutionContext &execution_context) {
	return make_unique<PhysicalCrossProductOperatorState>(execution_context, *this, children[0].get(), children[1].get());
}
