...
void DataChunk::Copy(DataChunk &other, idx_t offset) {
	...
	for (idx_t i = 0; i < ColumnCount(); i++) {
		D_ASSERT(other.data[i].buffer->vector_type == VectorType::FLAT_VECTOR);
...
void DataChunk::Append(DataChunk &other) {
	...
	for (idx_t i = 0; i < ColumnCount(); i++) {
		D_ASSERT(data[i].buffer->vector_type == VectorType::FLAT_VECTOR);
...
vector<LogicalType> DataChunk::GetTypes() {
	...
	for (idx_t i = 0; i < ColumnCount(); i++) {
		types.push_back(data[i].buffer->type);
	}
	return types;
}
...
void DataChunk::Serialize(Serializer &serializer) {
	...
	for (idx_t col_idx = 0; col_idx < ColumnCount(); col_idx++) {
		// write the types
		data[col_idx].buffer->type.Serialize(serializer);
	}
	// write the data
	for (idx_t col_idx = 0; col_idx < ColumnCount(); col_idx++) {
...
void DataChunk::Slice(DataChunk &other, const SelectionVector &sel, idx_t count, idx_t col_offset) {
	...
	for (idx_t c = 0; c < other.ColumnCount(); c++) {
		if (other.data[c].buffer->vector_type == VectorType::DICTIONARY_VECTOR) {
...
void DataChunk::ToArrowArray(ArrowArray *out_array) {
	...
	for (idx_t col_idx = 0; col_idx < ColumnCount(); col_idx++) {
		...
		switch (vector.buffer->vector_type) {
		...
		case VectorType::FLAT_VECTOR:
			...
			break;
		default:
			throw NotImplementedException(VectorTypeToString(vector.buffer->vector_type));
		}
